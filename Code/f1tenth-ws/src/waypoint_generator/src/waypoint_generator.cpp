...
WaypointGenerator::WaypointGenerator() : Node("waypoint_generator_node") {
    ...
    this->declare_parameter("save_path", "/home/f1tenth/f1tenth-ws/src/waypoint_generator/map/waypoints_odom.csv");
...
void WaypointGenerator::odom_callback(const nav_msgs::msg::Odometry::ConstSharedPtr odom_submsgObj) {
    ...
    if (diff > min_distance) {
        ...
        double linear_x = odom_submsgObj->twist.twist.linear.x; // linear.x
        double angular_z = odom_submsgObj->twist.twist.angular.z; // angular.z

        csv_odom << "\n" << x << ", " << y << ", " << linear_x << ", " << angular_z;
